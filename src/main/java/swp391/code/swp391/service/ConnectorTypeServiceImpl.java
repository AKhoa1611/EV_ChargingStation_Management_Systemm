...
public class ConnectorTypeServiceImpl implements ConnectorTypeService {
    ...
    public List<ConnectorTypeDTO> getConnectorTypesByChargingPointId(Long chargingPointId) {
        return connectorTypeRepository.findByChargingPointChargingPointId(chargingPointId).stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    }
    ...
    public ConnectorTypeDTO updateConnectorType(Long connectorTypeId, ConnectorTypeDTO connectorTypeDTO) {
        ...
        if (connectorTypeDTO.getChargingPointId() != null) {
            ChargingPoint chargingPoint = chargingPointRepository.findById(connectorTypeDTO.getChargingPointId())
                    .orElseThrow(() -> new RuntimeException("Charging point not found with id: " + connectorTypeDTO.getChargingPointId()));
            existingConnectorType.setChargingPoint(chargingPoint);
        } else {
            existingConnectorType.setChargingPoint(null);
        }
    ...
    public void deleteConnectorType(Long connectorTypeId) {
        ...
        if (connectorType.getVehicles() != null && !connectorType.getVehicles().isEmpty()) {
            throw new RuntimeException("Cannot delete connector type. It is being used by " +
                    connectorType.getVehicles().size() + " vehicle(s)");
        }
    ...
    private ConnectorType convertToEntity(ConnectorTypeDTO connectorTypeDTO) {
        ...
        if (connectorTypeDTO.getChargingPointId() != null) {
            ChargingPoint chargingPoint = chargingPointRepository.findById(connectorTypeDTO.getChargingPointId())
                    .orElseThrow(() -> new RuntimeException("Charging point not found with id: " + connectorTypeDTO.getChargingPointId()));
            connectorType.setChargingPoint(chargingPoint);
        }
    ...
    private ConnectorTypeDTO convertToDTO(ConnectorType connectorType) {
        ...
        dto.setVehicles(connectorType.getVehicles());
